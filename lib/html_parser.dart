...
class HtmlParser extends StatelessWidget {
  ...
  final int numberShortCharacterDisplay;
  var countNumber = 0;
  var subStringStart = '';
  var subStringEnd = '';
  var tagEnd = '';
  ...
  HtmlParser(
      ...
      this.numberShortCharacterDisplay = 2000})
  ...
  bool findSubStringInsert({StyledElement? cleanedTree}) {
    ...
    var listTagIgnor = ['img', 'svg', 'tr', 'td'];
    ...
    if (cleanedTree?.children.length == 0) {
      ...
      if ((countNumber + content.length) > numberShortCharacterDisplay) {
        subStringStart =
            content.substring(0, (numberShortCharacterDisplay - countNumber));

        subStringEnd = content.substring(
            (numberShortCharacterDisplay - countNumber), content.length);
        // subStringStart += '</${cleanedTree?.element?.localName ?? ''}>';
